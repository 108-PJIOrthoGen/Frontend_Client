...
/**
 * Read-only viewer of the AI treatment plan. Editing the AI answer was removed
 * — the doctor enters their own plan in the "Chẩn đoán bác sĩ" step.
 */
    ...
    <div className="flex-[3] bg-white border border-slate-200 rounded-2xl flex flex-col overflow-hidden shadow-xl shadow-slate-200/50">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar bg-slate-50/30">
        <Alert
          type="info"
          showIcon
          message="Phác đồ AI chỉ để tham khảo"
          description='Bạn sẽ nhập chẩn đoán và phác đồ của riêng mình ở bước tiếp theo — "Chẩn đoán bác sĩ".'
        />
        {surgeryPlan && <SurgerySection surgeryPlan={surgeryPlan} readOnly />}
        {systemicPlan && (
          <SystemicAntibioticTreatment guidelinePlan={systemicPlan} readOnly />
        )}
        {localPlan && <LocalAntibioticTreatment localPlan={localPlan} readOnly />}
